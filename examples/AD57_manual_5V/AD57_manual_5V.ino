/*****************************************************
 * AD57xx demo 0 - 5 volt with manual input
 *
 * Paul van Haastrecht
...
void setup() {
  ...
  Serial.begin(9600);
  serialTrigger(F("Press any key to begin:  0 - 5 volt with manual input."));
...
void loop() {
  ...
  // the values are written to the registers, BUT with
  ...
  // Unlike above we now load the values directly. This is default if we do
  // not provide the optinal bool OR set that to 1 (as we do later)
  ...
  // set lowest value. use the optional boolean as 1 to trigger
  // immediate loading (not necessary is default.)
  AD57.setDac(DAC_CHANNEL, 0x0,1);
...
/* calculate estimated voltage level (only postive)
 *
 * p5V          +5V
 * p10V         +10V
 * p108V        +10.8V
